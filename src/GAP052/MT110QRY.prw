...
Static Function MrkLegenda()
	...
	aAdd(aBrowse,{ .T.,  LoadBitmap(GetResources(), 'BR_AMARELO' ) , 'Solicitacao Parcialmente Atendida'                      , "C1_QUJE > 0 AND C1_QUJE <> C1_QUANT" } )
	...
	aAdd(aBrowse,{ .T.,  LoadBitmap(GetResources(), 'PMSEDT1'    ) , 'Solicitação Parcialmente Atendida Utilizada em Cotação' , "C1_TPSC <> '2' AND C1_QUJE > 0 AND C1_QUJE <> C1_QUANT AND C1_COTACAO <> SPACE(LEN(C1_COTACAO)) AND C1_COTACAO <> REPLICATE('X',Len(C1_COTACAO)) AND C1_IMPORT <>'S'" } )
